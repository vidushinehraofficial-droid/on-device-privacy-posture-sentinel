...
# winsound is Windows-only and does not exist on Linux (e.g. Streamlit Cloud).
# Import it only if available, and no-op otherwise so the app doesn't crash
# on deployment. Desktop/Windows users still get the beep as before.
try:
    import winsound
    HAS_WINSOUND = True
except ImportError:
    ...
    def analyze_posture(self, frame, sensitivity_angle=70, audio_enabled=True):
        ...
        if results.pose_landmarks:
            ...
            if angle < sensitivity_angle or angle > 110:
                status = "Slouching"
                line_color = (0, 0, 255)
                if audio_enabled and HAS_WINSOUND and (time.time() - self.last_beep_time > 3):
